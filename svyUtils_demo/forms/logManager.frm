encapsulation:60,
items:[
{
background:"#ffffff",
height:397,
partType:5,
typeid:19,
uuid:"02A718F0-D0DD-4890-B7D5-4F19FE2E71CB"
},
{
dataProviderID:"text",
displayType:1,
location:"129,173",
size:"363,80",
typeid:4,
uuid:"043A3022-A829-4051-920E-8A6ACFBDCEAA"
},
{
formIndex:7,
location:"300,136",
name:"labelError",
size:"395,20",
text:"you must specify a valid datasource in order to use the DBLogger",
transparent:true,
typeid:7,
uuid:"04A760CF-2E51-420C-A2CE-CA50991FF2C7",
visible:false
},
{
formIndex:1,
location:"25,136",
size:"109,20",
text:"DB Logger",
transparent:true,
typeid:7,
uuid:"12B8FB4B-5E94-4C92-BF11-3F7ACAD60F77"
},
{
location:"513,203",
onActionMethodID:"6D5E5A76-03EF-485C-8667-46ABC24C8EA2",
onDoubleClickMethodID:"-1",
onRightClickMethodID:"-1",
text:"warning",
typeid:7,
uuid:"49D81EFA-0011-4532-ADAE-810BB675DE5A"
},
{
dataProviderID:"DBLogEnabled",
displayType:4,
formIndex:6,
location:"139,136",
onDataChangeMethodID:"ED9003C2-1A30-4E2D-842B-0E9369728ED5",
text:"is enabled",
transparent:true,
typeid:4,
uuid:"5B97830A-CCD1-433D-8FDF-CEF87F8BB7B8"
},
{
fontType:"Segoe UI,0,26",
location:"26,19",
size:"363,56",
text:"Log Manager",
transparent:true,
typeid:7,
uuid:"5BC10646-FF18-4F50-AD60-1A5710797C05"
},
{
location:"26,272",
size:"765,20",
text:"The default application output logs messages in console or in server log for web clients and batch processors",
transparent:true,
typeid:7,
uuid:"63E521C8-9699-47F2-86D5-8EB231EB32CA"
},
{
dataProviderID:"fileLogEnabled",
displayType:4,
formIndex:6,
location:"139,106",
text:"is enabled",
transparent:true,
typeid:4,
uuid:"883181D9-6282-4D7D-8E9B-08BFBADA4107"
},
{
formIndex:1,
location:"25,106",
size:"109,20",
text:"File Logger",
transparent:true,
typeid:7,
uuid:"8A8A272D-00BE-4ABB-B494-4D38D99C1BEC"
},
{
location:"25,173",
size:"84,20",
text:"log message",
transparent:true,
typeid:7,
uuid:"D3E08E6B-93F1-4FD9-8972-EF948B0F8FF6"
},
{
location:"25,298",
size:"766,20",
text:"Enable the File Logger to append logs message to the file myLogs/myAppLog.log",
transparent:true,
typeid:7,
uuid:"DEC4746F-1447-4483-BD93-C679021F7755"
},
{
location:"513,233",
onActionMethodID:"74EEF732-DFF6-4002-B2F4-E1E4B977C612",
onDoubleClickMethodID:"-1",
onRightClickMethodID:"-1",
text:"error",
typeid:7,
uuid:"EB888266-6CC5-47EC-A74E-4E393A2C207F"
},
{
location:"513,173",
onActionMethodID:"051E959D-54F0-417F-93F2-E460354DE393",
onDoubleClickMethodID:"-1",
onRightClickMethodID:"-1",
text:"log",
typeid:7,
uuid:"EDF6CF38-2FB9-4DAF-9840-7344C3399620"
},
{
location:"25,324",
size:"766,20",
text:"Enable the DB logger to save the logs in a datasource. WARN: default datasource is undefined. is required to setup a valid datasource",
transparent:true,
typeid:7,
uuid:"FA1FCACF-3558-49A9-AF43-CBF59766CA51"
}
],
name:"logManager",
navigatorID:"-1",
onLoadMethodID:"-1",
showInMenu:true,
size:"818,465",
typeid:3,
uuid:"0A895371-C8C9-4AA3-A50D-D202D736A7CA"